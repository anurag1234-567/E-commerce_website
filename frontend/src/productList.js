    ...
    useEffect(()=>{
        const fetchData = async()=>{
            try{
                ...
                if(category) { url += `&category=${category}`; setPage(1); }
                if(sortBy) { url += `&sortBy=${sortBy}`; setPage(1); }
    ...
    return(
        <>
         ...
         <div className='items'>
            { data &&
              data.map((item, index) => {
                  return <div className='item' key={index} onClick={()=>{ handleClick(item._id) }}>
                           ...
                           <div className='detail-wrp'>
                               ...
                               <p className='detail-option'>View Details →</p>
                           </div>
                         </div>
              })
            }
